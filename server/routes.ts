...
import { uploadJobManager } from './upload-job-manager';
...
`;
  ...
  // ===========================================
  // UPLOAD JOB API ROUTES - Persistent Upload System
  // ===========================================

  // Create a new background upload job
  app.post('/api/upload-jobs', async (req, res) => {
    try {
      const userId = 'default-user';
      const { type, targetRepo, targetPath, files, metadata } = req.body;
      
      if (!type || !targetRepo || !files || !Array.isArray(files)) {
        return res.status(400).json({ error: 'Missing required parameters: type, targetRepo, files' });
      }
      ...
      for (const file of files) {
        if (!file.filePath || !file.fileName || !file.content || !file.encoding) {
          return res.status(400).json({ error: 'Invalid file format. Each file must have filePath, fileName, content, and encoding' });
        }
      }
      
      const job = await uploadJobManager.createUploadJob({
        userId,
        type: type as 'github_sync' | 'git_control',
        targetRepo,
        targetPath,
      ...
      res.json({ 
        job, 
        message: `Background upload job created with ${files.length} files. Upload will continue even if you navigate away.` 
      });
    } catch (error) {
      logger.error(`Failed to create upload job: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
  ...
  app.get('/api/upload-jobs/:jobId', async (req, res) => {
    try {
      const { jobId } = req.params;
      
      const job = await uploadJobManager.getUploadJob(jobId);
      if (!job) {
      ...
      logger.error(`Failed to get upload job: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
        error: 'Failed to get upload job',
        details: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });

  // Get upload job progress for real-time updates
  app.get('/api/upload-jobs/:jobId/progress', async (req, res) => {
    try {
      const { jobId } = req.params;
      
      const progress = await uploadJobManager.getJobProgress(jobId);
      res.json(progress);
    } catch (error) {
      logger.error(`Failed to get upload job progress: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
        error: 'Failed to get upload job progress',
        details: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  app.get('/api/upload-jobs', async (req, res) => {
    try {
      const userId = 'default-user';
      
      const jobs = await uploadJobManager.listUploadJobs(userId);
      res.json({ jobs });
    } catch (error) {
      logger.error(`Failed to list upload jobs: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
  ...
  app.delete('/api/upload-jobs/:jobId', async (req, res) => {
    try {
      const { jobId } = req.params;
      
      const cancelled = await uploadJobManager.cancelUploadJob(jobId);
      if (!cancelled) {
        return res.status(404).json({ error: 'Upload job not found or could not be cancelled' });
      }
      
      res.json({ message: 'Upload job cancelled successfully' });
    } catch (error) {
      logger.error(`Failed to cancel upload job: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
  ...
  app.post('/api/upload-jobs/:jobId/resume', async (req, res) => {
    try {
      const { jobId } = req.params;
      
      const resumed = await uploadJobManager.resumeUploadJob(jobId);
      if (!resumed) {
        return res.status(404).json({ error: 'Upload job not found or could not be resumed' });
      }
      
      res.json({ message: 'Upload job resumed successfully' });
    } catch (error) {
      logger.error(`Failed to resume upload job: ${error instanceof Error ? error.message : String(error)}`);
      res.status(500).json({ 
        error: 'Failed to resume upload job',
        details: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
