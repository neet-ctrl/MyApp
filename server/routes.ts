...
`;
  ...
  app.get('/api/settings/hardcoded-values', (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to fetch hardcoded values' });
    }
  });

  app.post('/api/settings/update-value', async (req, res) => {
    try {
      const { id, newValue } = req.body;
      
      if (!id || newValue === undefined) {
        return res.status(400).json({ error: 'ID and newValue are required' });
      }

      // Update value in actual files based on the ID
      await updateHardcodedValueInFiles(id, newValue);
      res.json({ success: true, message: 'Value updated successfully' });
    } catch (error) {
      console.error('Error updating value:', error);
      res.status(500).json({ error: 'Failed to update value in files' });
    }
  });

  // Function to update hardcoded values in actual files
  async function updateHardcodedValueInFiles(id: string, newValue: string) {
    const fs = require('fs').promises;
    const path = require('path');

    switch (id) {
      case 'telegram_api_id':
        // Update in auth-modal.tsx
        try {
          const authModalPath = path.join(process.cwd(), 'client/src/components/auth-modal.tsx');
          let content = await fs.readFile(authModalPath, 'utf8');
          content = content.replace(
            /apiId: import\.meta\.env\.VITE_TELEGRAM_API_ID \|\| '[^']*'/,
            `apiId: import.meta.env.VITE_TELEGRAM_API_ID || '${newValue}'`
          );
          await fs.writeFile(authModalPath, content, 'utf8');
          console.log(`Updated API ID in auth-modal.tsx to: ${newValue}`);
        } catch (error) {
          console.error('Failed to update API ID in auth-modal.tsx:', error);
        }
        break;
      ...
      case 'telegram_api_hash':
        ...
        try {
          ...
          content = content.replace(
            ...
            `apiHash: import.meta.env.VITE_TELEGRAM_API_HASH || '${newValue}'`
          );
          await fs.writeFile(authModalPath, content, 'utf8');
          console.log(`Updated API Hash in auth-modal.tsx to: ${newValue}`);
        } catch (error) {
          console.error('Failed to update API Hash in auth-modal.tsx:', error);
        }
        break;
      ...
      case 'telegram_phone':
        ...
        try {
          ...
          content = content.replace(
            ...
            `phoneNumber: import.meta.env.VITE_TELEGRAM_PHONE || '${newValue}'`
          );
          await fs.writeFile(authModalPath, content, 'utf8');
          console.log(`Updated phone number in auth-modal.tsx to: ${newValue}`);
        } catch (error) {
          console.error('Failed to update phone number in auth-modal.tsx:', error);
        }
        break;

      case 'github_pat':
        // Update in storage.ts
        try {
          const storagePath = path.join(process.cwd(), 'server/storage.ts');
          let content = await fs.readFile(storagePath, 'utf8');
          content = content.replace(
            /private defaultPAT: string = '[^']*'/,
            `private defaultPAT: string = '${newValue}'`
          );
          await fs.writeFile(storagePath, content, 'utf8');
          console.log(`Updated GitHub PAT in storage.ts to: ${newValue}`);
        } catch (error) {
          console.error('Failed to update GitHub PAT in storage.ts:', error);
        }
        break;

      default:
        console.log(`No update handler for ID: ${id}`);
    }
  }
