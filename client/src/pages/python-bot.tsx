...
export default function PythonBotPage() {
  ...
  const startBotMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Python Telethon Bot Started', description: 'Bot is now running with the exact same functionality as the original repository!' });
  ...
  const handleStart = () => {
    if (!apiId || !apiHash || !botToken || !authorizedUserId) {
      toast({ 
        ...
        <CardContent className="space-y-4">
          {status.running && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="font-medium">API ID:</span> {maskValue(status.apiId, showSecrets)}
              </div>
              <div>
                <span className="font-medium">API Hash:</span> {maskValue(status.apiHash, showSecrets)}
              </div>
              <div>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Configure your Telegram API credentials to run the Python telethon bot
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="apiId">Telegram API ID *</Label>
                <Input
                  id="apiId"
                  type="text"
              ...
              <div className="space-y-2">
                <Label htmlFor="apiHash">Telegram API Hash *</Label>
                <Input
                  id="apiHash"
                  type="password"
            ...
            <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm">
                  <p className="font-medium text-blue-800 dark:text-blue-200">How to get your credentials:</p>
                  <ul className="mt-2 space-y-1 text-blue-700 dark:text-blue-300">
                    <li>• API ID & Hash: Visit <a href="https://my.telegram.org" target="_blank" rel="noopener noreferrer" className="underline">my.telegram.org</a></li>
