...
class TelegramBot:
    ...
    async def start(self):
        """Start the Telegram bot with enhanced error handling for Railway deployment"""
        
        logger.logger.info("🚀 Attempting to start Telethon client...")
        logger.logger.info(f"Session file: {self.SESSION}")
        logger.logger.info(f"API ID configured: {'Yes' if self.API_ID else 'No'}")
        logger.logger.info(f"API Hash configured: {'Yes' if self.API_HASH else 'No'}")
        logger.logger.info(f"Bot token configured: {'Yes' if self.BOT_TOKEN else 'No'}")
        
        try:
            # Enhanced error handling for Telegram client connection
            logger.logger.info("📡 Connecting to Telegram servers...")
            await self.client.start(bot_token=str(self.BOT_TOKEN))
            logger.logger.info("✅ Successfully connected to Telegram!")
            ...
            try:
                ...
                logger.logger.info("📨 Welcome message sent successfully")
            except Exception as msg_error:
                logger.logger.error(f"❌ Failed to send welcome message: {msg_error}")
            ...
            try:
                ...
                logger.logger.info("📥 Pending messages processed")
            except Exception as pending_error:
                logger.logger.error(f"❌ Failed to process pending messages: {pending_error}")

            # Keep bot running
            logger.logger.info("🔄 Bot is now running and listening for messages...")
            await self.client.run_until_disconnected()
            
        except Exception as e:
            logger.logger.error(f"💥 CRITICAL: Failed to start Telegram client: {e}")
            ...
            try:
                ...
                logger.logger.info("✅ Network connectivity to Telegram servers OK")
            except Exception as net_error:
                logger.logger.error(f"❌ Network connectivity issue: {net_error}")
